...
def read_embeddings(embeddings_path, vocab_path):
  ...
  Args:
    embeddings_path: full path for the embeddings file, where embeddings file
      contains word and its vector(float values) per line, separated by blank space.
    vocab_path: full path for the vocab file, where each line contains a single 
      vocab word.
      
  Returns:
    a 2d array where row index corresponds to the word index in the vocab (special
    vocab and other unknown words are also included at their respective row index.
  """
